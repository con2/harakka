...
import { AddItemProps, ItemFormTag } from "@/types";
...
function AddItemForm(props: AddItemProps) {
  ...
  const { storage, tags: popularTags } = props;
  ...
  function onSubmit(values: z.infer<typeof createItemDto>) {
    console.log("submit values: ", values);
  }

  useEffect(() => {
    console.log("Form update: ", form, [form]);
  }, [form]);
  ...
  const toggleTag = (e: React.MouseEvent<HTMLElement>) => {
    ...
    if (exists) {
      ...
      const newTag = popularTags.find((t) => t.id === id);
      if (newTag)
        setSelectedTags([
          ...selectedTags,
                  ...
                  <FormItem>
                    ...
                    <Select>
                      <SelectTrigger
                        ...
                        disabled={storage === undefined || Boolean(storage)}
                      >
                        <SelectValue
                          className="border shadow-none border-grey"
                          placeholder={
