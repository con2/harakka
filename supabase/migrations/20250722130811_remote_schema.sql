...
CREATE OR REPLACE FUNCTION "public"."auth_hook_get_user_roles"("event" "jsonb") RETURNS "jsonb"
...
BEGIN
  ...
  -- If no user_id provided, return empty roles
  ...
  -- Fetch user roles with proper table aliases to avoid ambiguity
  FOR role_record IN
    SELECT 
      uor.id,
      uor.user_id,
    ...
    WHERE uor.user_id = input_user_id  -- Use fully qualified column name
      AND uor.is_active = true
    ORDER BY uor.created_at DESC
  LOOP
    -- Build role object and add to array
    roles_array := roles_array || jsonb_build_object(
      ...
      'organization_slug', role_record.organization_slug
    );
  END LOOP;
  ...
  RETURN jsonb_build_object(
    'app_metadata', jsonb_build_object(
      'roles', roles_array,
      'role_count', jsonb_array_length(roles_array),
      'last_role_sync', NOW()
    )
  );
...
ALTER FUNCTION "public"."auth_hook_get_user_roles"("event" "jsonb") OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."handle_user_role_change"() RETURNS "trigger"
  ...
  role_count INTEGER;
BEGIN
  -- If this is a DELETE operation, use OLD.user_id
  -- For INSERT operations, use NEW.user_id
  -- For UPDATE operations, use either OLD.user_id or NEW.user_id (they should be the same for role changes)
  ...
  -- Get the current roles for this user from the user_organization_roles table
  -- Using the actual column names from your tables
  WITH user_roles AS (
    SELECT 
      user_organization_roles.id,
      user_organization_roles.created_at,
      roles.role as role_name,  -- Using 'role' column instead of 'name'
      organizations.id as organization_id,
      organizations.name as organization_name,
      roles.id as role_id
    FROM 
      public.user_organization_roles
      JOIN public.roles ON user_organization_roles.role_id = roles.id
      JOIN public.organizations ON user_organization_roles.organization_id = organizations.id
    WHERE 
      user_organization_roles.user_id = user_id_val
  )
  SELECT 
    jsonb_agg(
      jsonb_build_object(
        ...
        'role_name', user_roles.role_name,  -- Using the alias we defined above
        'organization_id', user_roles.organization_id,
        'organization_name', user_roles.organization_name,
  ...
  -- Update the app_metadata in auth.users to include the updated roles
  UPDATE auth.users 
      ...
      CASE 
        WHEN raw_app_meta_data IS NULL THEN 
          jsonb_build_object(
            ...
            'refresh_required', extract(epoch from now())::text
          )
        ELSE
...
CREATE OR REPLACE FUNCTION "public"."update_jwt_on_role_definition_change"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  affected_user_id UUID;
BEGIN
  -- When a role definition changes, update JWT for all users with that role
  FOR affected_user_id IN 
    SELECT DISTINCT uor.user_id 
    FROM user_organization_roles uor 
    WHERE uor.role_id = COALESCE(NEW.id, OLD.id)
      AND uor.is_active = true
  LOOP
    -- Trigger the JWT update for each affected user
    PERFORM update_user_jwt_on_role_change_for_user(affected_user_id);
  END LOOP;
...
ALTER FUNCTION "public"."update_jwt_on_role_definition_change"() OWNER TO "postgres";
...
CREATE OR REPLACE FUNCTION "public"."update_user_jwt_on_role_change"() RETURNS "trigger"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$DECLARE
  affected_user_id UUID;
  user_roles JSONB;
BEGIN
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        'id', uor.id,
        'role_name', r.role,                    -- Changed: role_name -> name
        'organization_id', uor.organization_id,     -- Changed: organization_id -> org_id
        'organization_name', o.name,                -- Changed: organization_name -> org_name
        'role_id', uor.role_id,
...
ALTER FUNCTION "public"."update_user_jwt_on_role_change"() OWNER TO "postgres";


CREATE OR REPLACE FUNCTION "public"."update_user_jwt_on_role_change_for_user"("target_user_id" "uuid") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    AS $$
DECLARE
  user_roles JSONB;
...
ALTER FUNCTION "public"."update_user_jwt_on_role_change_for_user"("target_user_id" "uuid") OWNER TO "postgres";
...
CREATE OR REPLACE TRIGGER "refresh_jwt_on_role_change" AFTER INSERT OR DELETE OR UPDATE ON "public"."user_organization_roles" FOR EACH ROW EXECUTE FUNCTION "public"."handle_user_role_change"();
...
CREATE OR REPLACE TRIGGER "trigger_update_jwt_on_role_change" AFTER INSERT OR DELETE OR UPDATE ON "public"."user_organization_roles" FOR EACH ROW EXECUTE FUNCTION "public"."update_user_jwt_on_role_change"();



CREATE OR REPLACE TRIGGER "trigger_update_jwt_on_role_definition_change" AFTER DELETE OR UPDATE ON "public"."roles" FOR EACH ROW EXECUTE FUNCTION "public"."update_jwt_on_role_definition_change"();
...
GRANT ALL ON FUNCTION "public"."auth_hook_get_user_roles"("event" "jsonb") TO "anon";
GRANT ALL ON FUNCTION "public"."auth_hook_get_user_roles"("event" "jsonb") TO "authenticated";
GRANT ALL ON FUNCTION "public"."auth_hook_get_user_roles"("event" "jsonb") TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_jwt_on_role_definition_change"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_jwt_on_role_definition_change"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_jwt_on_role_definition_change"() TO "service_role";
...
GRANT ALL ON FUNCTION "public"."update_user_jwt_on_role_change"() TO "anon";
GRANT ALL ON FUNCTION "public"."update_user_jwt_on_role_change"() TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_user_jwt_on_role_change"() TO "service_role";



GRANT ALL ON FUNCTION "public"."update_user_jwt_on_role_change_for_user"("target_user_id" "uuid") TO "anon";
GRANT ALL ON FUNCTION "public"."update_user_jwt_on_role_change_for_user"("target_user_id" "uuid") TO "authenticated";
GRANT ALL ON FUNCTION "public"."update_user_jwt_on_role_change_for_user"("target_user_id" "uuid") TO "service_role";
